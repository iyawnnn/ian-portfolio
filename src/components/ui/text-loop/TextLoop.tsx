...
const TextLoop = ({
  ...
  // Just the masked glyph — no ref here. Kept separate from the
  // `<foreignObject>` wrapper below so the ref callback that writes into
  // `headMarkRefs`/`tailMarkRefs` can stay inline at its own JSX call
  // site: the `react-hooks/refs` lint rule flags a ref value crossing a
  // function-call boundary (even via a setter closure) as an unsafe
  // render-time access, so each `.map()` is written out directly instead
  // of factored through a shared helper that takes the ref array.
  const markGlyphStyle: CSSProperties = {
